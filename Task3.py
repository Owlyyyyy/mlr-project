"""
Part III: Font Image Classification
ECONM0011 Machine Learning for Economics

This script implements the full pipeline:
  Step 1: Image preprocessing and data construction
  Step 2: CNN training with embedding layer
  Step 3: Evaluation (top-1/top-5 prototypical accuracy + pairwise ROC)

Hardware: Designed for MacBook M1 Pro 16GB (uses MPS acceleration)
Dataset: Subsampled from Font_Images.zip (192,384 fonts → 300 fonts)
"""
...
CROPS_PER_IMAGE = 20                  # random square crops per font image
...
# ============================================================
# DEVICE SETUP (MPS for M1, fallback to CPU)
# ============================================================
...
def crop_sentences(img):
    """Provided Crop.py: split a 400x400 font image into 2 sentence crops."""
...
all_crops = []  # list of (crop_array, font_label)
...
class FontCNN(nn.Module):
    ...
    def get_embedding(self, x):
        """Extract the embedding (for prototypical evaluation)."""
        x = self.features(x)
        x = x.view(x.size(0), -1)
        x = self.dropout(self.relu1(self.fc1(x)))
        emb = self.relu2(self.embedding(x))
...
# --- 3a: Extract all test embeddings ---
...
# --- 3a-i: Top-1 and Top-5 Accuracy (Prototypical Network Method) ---
...
for c in valid_classes:
    ...
    for other_c in valid_classes:
        ...
        if other_c == c:
            # Use the support set (excluding query) for this class
            proto = prototype_c
        else:
...
# --- 3a-ii: Pairwise ROC Curve ---
print("\nComputing pairwise ROC curve...")
print("(This may take a moment for large test sets...)")

# To keep computation manageable, sample pairs if test set is large
# For N test samples, there are N*(N-1)/2 pairs
